...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    if (r === 'vendeur') {
      setRole('vendeur');
      setEmail(''); // Pre-fill with mockup default
    } else {
      setRole('acheteur');
      setEmail(''); // Pre-fill with mockup default
    }
  ...
  useEffect(() => {
    ...
    const nextPath = user.status === 'brouillon' && user.emailVerified
      ? localizedPath(`/register?step=documents&role=${user.role}`, language)
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nextPath = res.user.status === 'brouillon' && res.user.emailVerified
        ? localizedPath(`/register?step=documents&role=${res.user.role}`, language)
      ...
      if (err.code === 'auth.email_not_verified') {
        ...
        setTimeout(() => {
          router.push(localizedPath(`/register?step=otp&role=${role}`, language));
        }, 2000);
        ...
        <div className="w-full max-w-[440px] flex flex-col justify-center">
          ...
          <form onSubmit={handleLogin} className="space-y-[18px]">
            ...
            <div className="flex justify-end text-[13px]">
              <Link href={localizedPath('/forgot-password', language)} className="text-[#8a8270] hover:underline">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full h-[52px] rounded-[9px] text-white text-[15px] font-bold uppercase tracking-[0.03em] flex items-center justify-center transition disabled:opacity-50 select-none cursor-pointer ${role === 'acheteur'
          ...
          <div className="text-center text-[14px] text-[#5a5e66] mt-[22px]">
            {t('login.noAccount')}{' '}
            <Link href={localizedPath(`/register?role=${role}`, language)} className="text-[#d9704f] font-semibold hover:underline">
