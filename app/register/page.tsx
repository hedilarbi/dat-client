...
function RegisterContent() {
  ...
  const [activityType, setActivityType] = useState('Garagiste');
  ...
  useEffect(() => {
    ...
    if (r === 'vendeur') {
      setRole('vendeur');
      setActivityType('Centre VHU');
    } else {
      setRole('acheteur');
      setActivityType('Garagiste');
    }
  ...
  useEffect(() => {
    ...
    if (requestedStep !== 'documents' || !user || !user.emailVerified) return;

    setRole(user.role === 'vendeur' ? 'vendeur' : 'acheteur');
    setActivityType(user.activityType || (user.role === 'vendeur' ? 'Centre VHU' : 'Garagiste'));
    setEmail(user.email || '');
    ...
    <div className="max-w-[1240px] w-full bg-white rounded-[10px] overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.14)] font-sans text-black flex flex-col">
      ...
      {step === 1 && (
        <form onSubmit={handleStep1Submit} className="p-6 sm:p-9 lg:p-[36px_48px_40px] space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block text-[12px] font-semibold text-[#4c5058] mb-2">{t('register.activityType')}</label>
              <select
                value={activityType}
          ...
          <div className="pt-4 border-t border-[#efece3] flex justify-between items-center">
            ...
            <button
              type="submit"
              disabled={loading}
              className="h-12 px-8 bg-[#13243c] hover:bg-slate-800 text-white font-bold rounded-[9px] uppercase tracking-[0.03em] transition disabled:opacity-50 select-none cursor-pointer"
            >
        ...
        <div className="p-6 sm:p-9 lg:p-[36px_48px_40px] bg-white min-h-[460px]">
          <form onSubmit={handleVerifyOtpSubmit} className="w-full bg-white text-center">
            ...
            <button
              ...
              className="w-full h-[52px] rounded-[9px] bg-[#d9704f] hover:bg-[#c26040] text-white text-[15px] font-bold uppercase tracking-[0.03em] mb-4 disabled:opacity-50 select-none cursor-pointer"
            >
